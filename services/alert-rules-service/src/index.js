...
app.get('/health', (req, res) => res.json({ status: 'ok', service: 'alert-rules-service' }));
...
connectDB().then(() => {
  ...
  server.listen(PORT, () => {
    console.log(`[alert-rules-service] listening on :${PORT} (${process.env.SSL_KEY_PATH ? 'https' : 'http'})`);
  });
});
